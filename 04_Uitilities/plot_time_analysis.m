function plot_time_analysis(stallLegends,StallTime)
    bldspn = [10.25 18.45 22.55 30.75 38.95 43.05 51.25 57.40 60.13];
    WSP = 4:2:14;
    colors = turbo(15);
    folders = length(StallTime(1,1,:));
    % Plotting
    ...
    try
        for p = WSP(1):(WSP(2)-WSP(1)):WSP(end)
                ...
                ylim(ax,[0 40]);
                title(ax, num2str((p)) + " m/s");
            hold(ax,'off');
            savefig(fignear,"figures\NearStallAnalysis.fig");
        end
    catch ME
        fprintf('Error in Stall time analysis for %s: %s\n', fieldName, ME.message);
    end
end
